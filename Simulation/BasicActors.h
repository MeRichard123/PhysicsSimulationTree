...
namespace PhysicsEngine
{
	...
	public:
		...
		void SetDriveForceLimit(PxReal value)
		{
			((PxRevoluteJoint*)joint)->setDriveForceLimit(value); // Adjust as needed
		}
		...
		void SetLimits(PxReal lower, PxReal upper, PxReal damping)
		{
			PxJointAngularLimitPair limitPair(lower, upper, 0.01f);
			limitPair.damping = damping;
			((PxRevoluteJoint*)joint)->setLimit(limitPair);
	...
	public:
		...
		// Method to set the joint drive velocity
		void DriveVelocity(PxReal value)
		{
			// Wake up the attached actors
			PxRigidDynamic* actor_0, * actor_1;
		...
		// Method to set angular limits for the spherical joint
		void SetLimits(PxJointLimitCone limit)
		{
			((PxSphericalJoint*)joint)->setLimitCone(limit);
